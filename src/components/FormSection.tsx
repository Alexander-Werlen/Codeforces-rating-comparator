...
export const FormSection: React.FC<Props>  = ({handleSubmit, newHandle, loadingNewUser}) => {
    return (
        ...
        <form className="mt-6 mb-2" onSubmit={handleSubmit}>
            ...
            {!loadingNewUser && 
            <input type="submit" value="ADD" className="ml-4 w-14 sm:w-20 border-2 rounded p-1 cursor-pointer bg-blue-500 border-blue-500 hover:bg-blue-600 hover:border-blue-600 text-white"/>
            }
            {loadingNewUser && 
            <Button disabled className="ml-4 h-9 w-14 sm:w-20 border-2 rounded p-1 bg-blue-500 border-blue-500 text-white text-base">
            ADD
